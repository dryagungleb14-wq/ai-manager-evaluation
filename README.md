...
Проект использует автоматические миграции при запуске сервера. При каждом старте приложения:
1. Создаются таблицы, если их нет (через `CREATE TABLE IF NOT EXISTS`)
2. Выполняются проверки схемы и добавляются отсутствующие столбцы

Это обеспечивает:
- **Автоматическое обновление** схемы при деплое новых версий
- **Идемпотентность** — безопасно запускать многократно
- **Отсутствие ручных операций** — миграции применяются автоматически

Подробности о текущих миграциях см. в [`MIGRATION_TRANSCRIPT_ID.md`](./MIGRATION_TRANSCRIPT_ID.md) и каталоге `migrations/`.

Альтернативный способ синхронизации схемы (для PostgreSQL):
```bash
npm run db:push
```
**Примечание:** Команда требует переменную окружения `DATABASE_URL`.
